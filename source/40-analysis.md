...
한국 아파트에 대한 많은 선행연구에서는
10년 단위 연대를 둘로 나눈 5년 단위 전후반을 기준으로 시기를 설정한다.
...
박인석 등 [-@박인석2014전용면적]은
1998년 전용면적 산정기준 변경과
2006년부터 발코니 확장이 허용된 것의
영향을 분석하면서
5년 단위 전후반으로 시기를 설정하였다.
...
1980년대 후반|1985--1989|목동 신시가지 (1983), 3저 호황 (1986--1989), 발코니 외벽 기준 면적 산입 제외 (1988)
1990년대 전반|1990--1994|수도권 1기 신도시 (1988--), 발코니 창호 설치 허용 (1993)
1990년대 후반|1995--1999|외환위기 (1997), 분양가 자율화 (1999)
2000년대 전반|2000--2004|탑상형 아파트 도입 (2002), 발코니 창호 설치 의무화 (서울시, 2003)
...
<!-- 1986년 발코니 깊이 1.2 m 까지 바닥면적 산입 제외,
1988년 1.5 m까지 바닥면적 산 입 제외 및 실내공간화 허용,
2000년 화단형 발코니에 대 한 깊이 2m까지 바닥면적 산입 제외,
2005년 침실확장 허용 등 1980년대 후반부터 본격화된 발코니 면적 규제 완화
박인석 등 [-@박인석2014전용면적] -->
...
딥 러닝 모형 내부에서는
입력된 단위평면마다 각 범주와의 관계에 대한 점수가 매겨진다.
분류 모형의 출력층에서 각 범주별로 점수가 도출되고,
...
해당 범주에 가장 잘 맞는 단위평면이라고 말할 수 있다.

그러나, 한 단위평면이 여러 범주에서 동시에 가장 높은 점수를 얻는 경우도 있다.
이런 경우 단순히 하나의 범주에서 높은 점수를 얻는다는 것이
가장 확실하게, 다른 범주가 아니라 그 범주라고 분류된 것은 아니다.
이 경우 아무리 높은 점수를 받더라도
단 하나의 범주를 대표하는 사례로는 부적합하다고 판단할 수 있다.
딥 러닝 분류 모형은 마지막 단계에서 점수가 가장 높은 범주 하나만 골라 출력하기 때문에
이러한 문제가 자연스럽게 해결되지만
점수를 그대로 사용한다면 추가적인 처리가 필요하다.
예를 들어, 가장 높은 점수에서 나머지 범주의 점수를 뺀 값을
가장 확실하게 해당 범주로 분류된 정도를 대표하는 값으로 사용하여,
대표 단위평면을 도출할 수 있다.
...
따라서 시기별 분류 모형의 경우에는
가장 높은 점수에서 나머지 전체 점수가 아니라 직전 시기의 점수만 뺀 값을 기준으로 사용하여,
해당 시기 점수가 높고 직전 시기 점수는 낮은 단위평면을
해당 시기에 새롭게 등장한 단위평면의 대표로 삼는다.
