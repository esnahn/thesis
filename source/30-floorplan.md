...
![실 이름 오류 (오른쪽 침실)](fp_label_error.jpg){height=25%}
...
다음으로, 각 실을 나타내는 색을 기준으로 실내가 확실한 영역을 찾고,
...
![실 영역 및 경계 인식](fp_zones.png){height=25%}
...
입력값의 크기도 기존 모형의 2배/절반 단위로 결정하는 것이 모형을 데이터셋에 맞추기에 유리하다.
...
모든 평면을 손실 없이 담아낼 수 있으며,
거기서 더 절반으로 줄인 56픽셀은 11.2미터로
대부분의 평면에서 실내 영역에 해당하는 부분에 집중하여 분석할 수 있다.
두 가지 크기의 이미지에 대한 분석을 모두 수행할 수 있도록
단위평면 데이터셋에서 공간배치 데이터는 (112\times 112\times 6) 크기의 행렬로 표현한다.

![단위평면의 폭 및 깊이 분포](fp_size_kde.pdf){#fig:fp_size}

서로 다른 면적의 단위평면을 비교하기 위하여
주 향 발코니 중앙을 기준점으로 삼아,
단위평면의 주 향이 위치한 아래쪽을 기준으로 상하 정렬하고,
좌우는 가운데로 정렬한다.

이와 같이 처리된 단위평면 공간배치의 인식 결과물은 [@fig:recog]와 같다.
...
한국 아파트의 전용면적은 국민주택 규모 기준인 85 제곱미터와 60 제곱미터에 집중되어 있다.
그보다 큰 85 제곱미터 이상 중대형/대형 평면에서는 전용면적이 다양하게 나타난다. (@fig:area)
대형 평면의 전용면적 범위를 선형적으로 정규화하면,
평면 유형의 대부분이 해당하는 60과 85 제곱미터의 차이가 크게 나타나지 않는다.
따라서
최빈값인 85 제곱미터를 중심으로,
소형 평면과 대형 평면이 같은 간격만큼 떨어지도록 하여
비율 변수의 특성을 살리는 정규화를 적용할 필요가 있다.

![전용면적 분포](area_kde.pdf){#fig:area}

이를 위하여
면적에 로그를 적용하여 대형과 소형의 균형을 조절하고,
로그 함수의 밑을 $85 m^2$와 $60 m^2$의 비율 차이로 정의하여
두 규모의 간격을 1로 정의하는 로그 정규화를 적용한다.
로그 척도에서는 규모의 차이가 비율로 계산되기 때문에
평균을 구하면 85 제곱미터를 변환한 것과 유사한 값을 보인다.
85 제곱미터를 0으로 정의하면 0을 중심으로 한 정규화가 가능하다.
