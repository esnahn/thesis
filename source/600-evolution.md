...
본 연구에서 귀납적으로 도출된 공간배치 유형을 비교하였다.

이를 위하여,
선행연구의 기준에 따라 분류된
다양한 한국 아파트 단위평면 사례가
해당 시기의 공간배치 유형과 맺는 관계를 분석하였다.
BAM 시각화를 적용하여,
각 평면이
해당 시기의 공간배치 유형과 연관된 영역을 시각화하였다.
이러한 시각화를 통하여,
각 유형이 인식한
해당 시기의 공간배치 특성이
한국 아파트 계획의
...
신경망 연결의 가중치는
단위평면과
시기별 공간배치 특성 사이의 관계를 재현하고 있다.
딥 러닝 모형의 입력 계층에 단위평면이 입력되면,
출력 계층의 직전 계층인
잠재표현 계층은
시기 구분과 관련된 전반적인 공간배치를 인식한 결과를 나타내게 된다.

이렇게 단위평면의 공간배치를 인식하는 잠재표현 계층은
서로 독립적으로 다른 공간배치 특성을 인식하는 여러 노드로 이루어져 있다.
잠재표현 계층의 각 노드는
입력 계층에 입력된 단위평면에서
해당 공간배치 특성이 나타날 때
신경망 연결에 따른 계산의 결과로 높은 값을 가지게 되는데,
이러한 상태를 활성화된다고 표현한다.
활성화된 노드는 이어지는 계층에 더 강한 영향을 행사할 수 있다.
잠재표현 계층은 출력 계층 직전에 위치하고 있으므로,
잠재표현 계층의 노드가 활성화된다는 것은
단위평면에서 나타난 공간배치 특성을 인식하여
이를 기반으로 준공 시기를 구분할 수 있음을 의미한다.
...
[-@sec:model]절에서 구축한
수정 VGG-GAP 모형의 잠재표현 노드의 활성화 사이의 관계는
...
높은 활성화 (밝은 노랑)를 보이고,
나머지 영역은 낮은 활성화 (어두운 파랑) 정도가 나타나는 것을 의미한다.
그러나 실제 결과에서는,
...
단위평면 사례와
해당 시기 공간배치 유형 모두에 대한 관계를 종합적으로 분석하였다.
[-@sec:bam]절에서
BAM 시각화를 위하여 도출된
각 공간배치 유형별
단위평면 사례를 대상으로,
...
주요한 공간배치 특성으로 지목하였다
([@fig:evol80l]).
이러한 공간배치는 복도형 주동에서만 나타난 것은 아니었다.
1990년대 전형적인 계단실형 단위평면
([@fig:evol90m;@fig:evol90l]) 에서
현관에서 먼 쪽 전면에 주 침실이 배치되었으며,
